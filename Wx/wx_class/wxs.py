...
class XMLParse:
    ...
    def generate(self, timestamp, **kwargs):
        ...
        # 基金click
            ...
            assig = models.Fund.objects.all().values('fund_code', 'fund_invest', 'fund_get_state')
            lis = []
            for ke in assig:
                if ke['fund_get_state']:
                    lis.append(ke['fund_code'])
            fund_spider.Grab(lis).get_fund_info()
            fu = models.Fund.objects.filter(fund_get_state=True).values_list('fund_name', 'fund_rise_fall', 'fund_pic_url', 'fund_invest', 'fund_get_state')
            d = defaultdict(list)
            ...
            for _ in d.values():
                items += item.format(p=_[0], n='涨' if _[0][3]*_[0][1]/100 > 0 else '跌', m=_[0][3]*_[0][1]/100, k='盈' if _[0][3]*_[0][1]/100 > 0 else '亏')
...
if __name__ == "__main__":
    ...
    # import time
    # p = XMLParse().generate(int(time.time()), **{'FromUserName': 1, 'ToUserName': 2, 'MsgType': 'event', 'EventKey': 'V1001_fund'})
    # print(p)
